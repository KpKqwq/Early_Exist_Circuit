...
class BiDirectional_CGRS_LogitsProcessor:
    def __init__(self, monitor_ref, stop_token_id, tokenizer):
        self.monitor = monitor_ref
        self.stop_token_id = stop_token_id
        self.tokenizer = tokenizer

        self.reflection_words = ["Wait", "But", "Alternatively", "Let", "Hold"]
        self.reflection_ids = []
        for word in self.reflection_words:
            self.reflection_ids.extend(tokenizer.encode(word, add_special_tokens=False))
            self.reflection_ids.extend(tokenizer.encode(" " + word, add_special_tokens=False))
        self.reflection_ids = list(set(self.reflection_ids))
    ...
    def __call__(self, input_ids, scores):
        ...
        last_token_id = input_ids[-1] if len(input_ids) > 0 else -1
        is_at_sentence_end = (last_token_id == self.newline_id)

        if self.monitor.triggered:
            scores.fill_(-float("inf"))
            scores[self.stop_token_id] = 1000.0
            return scores

        elif sim > 0.45:
            scores[self.reflection_ids] = -float("inf")

        elif 0.3 < sim < 0.45 and mag < 0.2 and is_at_sentence_end:
            for rid in self.reflection_ids:
                scores[rid] += 3.0
            print(f"[Boost] Sim={sim:.2f}, Mag={mag:.2f}")
...
class CGRS_NeuronLogitsProcessor:
    def __init__(self, monitor_ref, stop_token_id, tokenizer):
        self.monitor = monitor_ref
        self.stop_token_id = stop_token_id
        self.tokenizer = tokenizer
    ...
    def __call__(self, input_ids, scores):
        ...
        if self.monitor.triggered:
            ...
            print(f"CGRS Suppressed! Sim={sim:.2f}, Mag={mag:.2f}")
...
def ensemble_mlp_hook(monitor, layer_idx, module, input, output):
    with torch.no_grad():
...
def register_vllm_hooks(monitor, llm_engine):
    try:
    ...
    for layer_idx in monitor.target_layers:
        hook_fn = partial(ensemble_mlp_hook, monitor, layer_idx)
        layers[layer_idx].mlp.act_fn.register_forward_hook(hook_fn)

    print(f"Hook Registered on {len(monitor.target_layers)} layers.")
    return True
